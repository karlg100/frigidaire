...
Frigidaire.prototype.scheduleUpdates = function(applianceId, callback) {
  ...
  var sleepTime = 5000+Math.floor(Math.random() * Math.floor(500)); // we need some randomness, otherwise one will always fail to run
  ...
  //self.updateTimer.push(setInterval(function(){self.getTelemUpdate(applianceId, callback)}), 5000);
}
