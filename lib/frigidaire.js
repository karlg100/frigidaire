...
Frigidaire.prototype.get = function(endpoint, args, callback, dataType, timeout) {
  ...
  if ('undefined' === typeof SEND_REQUEST || SEND_REQUEST) {
    request.post({url: url, headers: headers, form: form, jar: true, json: 'json' == dataType}, function(err, response, body) {
      ...
      if (response.statusCode == 401 && (self.deviceToken || self.instanceId)) {
...
Frigidaire.prototype.getValue = function(applianceId, attribute, callback, skipUpdate = false) {
  ...
  if (!self.instanceId) {
    ...
    debug('calling getTelemUpdate()');

    return self.getTelemUpdate(applianceId, function(err, data) {
      if (err) {
        return callback(err);
      }
      return self.getValue(applianceId, attribute, callback, true);
    });
  }
