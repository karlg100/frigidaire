...
*/
 ...
 Frigidaire.prototype.getValue = function (applianceSn, attribute, callback, attempt = 0) {
     ...
     if (!this.telemPopulated()) {
         ...
         return setTimeout(function () { self.getValue(applianceSn, attribute, callback, attempt) }, sleepTime);
     }
 ...
 Frigidaire.prototype.getMode = function (applianceSn, callback, attempt = 0) {
     debug("getMode()");
     var self = this;

     ++attempt;
     if (attempt > MAX_RETRIES) {
         callback("getMode() - max retries exceeded", null);
         return;
     }
     ...
     if (!this.telemPopulated()) {
         ...
         return setTimeout(function () { self.getMode(applianceSn, callback, attempt) }, sleepTime);
     }
 ...
 Frigidaire.prototype.getFanMode = function (applianceSn, callback, attempt = 0) {
     ...
     if (!this.telemPopulated()) {
         ...
         return setTimeout(function () { self.getFanMode(applianceSn, callback, attempt) }, sleepTime);
     }
