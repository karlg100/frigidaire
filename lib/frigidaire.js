...
function Frigidaire(options) {
  ...
  this.userAgent = opts.userAgent || 'ELXSmart/4.0.2 (iPad; iOS 11.4; Scale/2.00),ELXSmart/4.0.1 (iPad; iOS 11.4; Scale/2.00),Mozilla/5.0 (iPad; CPU OS 11_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15F79/Worklight/7.1.0.0 (4311995584)';
  ...
  this.UNIT=7005;
  this.FANMODE=7006;
  // 7007 - Current Fan Running Level
  // 7008 - Model
  // 7009 - Serial Number
  // 7010 - NIU Version
  this.COOLINGSTATE=7011;    // currently cooling / not cooling
  // 7013 - unknown
  // 7020 - unknown
  // 7024 - unknown
  // 7025 - unknown
...
Frigidaire.prototype.getTelemUpdate = function(applianceId, callback) {
  ...
  debug(self.lastUpdate.toISOString());
  debug(self.lastUpdate.toISOString().replace('T','-').replace(':','.').replace('Z','000'));
...
Frigidaire.prototype.getCoolingState = function(applianceId, callback) {
  ...
  debug("getting cooling state for "+applianceId);
  this.getValue(applianceId, this.COOLINGSTATE, function(err, result) {
    if (err) {
      return callback(err);
    }
    return callback(null, result);
